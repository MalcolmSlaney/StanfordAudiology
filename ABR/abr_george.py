...
def read_mouse_exp(filename: str) -> MouseExp:
    ...
    all_data_rows = []
    with open(
        filename, "r", encoding="latin-1", newline="", errors="replace"
    ) as csvfile:
    ...
    for d in all_exp_dirs:
        try:
            ...
            if not all_exps:
                ...
                print(
                    "  Channels: " f"{sorted(list(set([e.channel for e in all_exps])))}"
                )
                print(
                    "  Frequencies: " f"{sorted(list(set([e.freq for e in all_exps])))}"
                )
                # break
...
def cache_all_mouse_dir(
    ...
    Args:
      waveform_dir:  Where to find the experimental for this animal
    ...
    print("Cache_all_mouse_dir from", waveform_dir, max_files, max_bytes)
    all_exp_files = [
        f
        for f in os.listdir(waveform_dir)
        if os.path.isfile(os.path.join(waveform_dir, f)) and f.endswith(".csv")
    ]
    ...
    for f in all_exp_files:
        ...
        exp = read_mouse_exp(os.path.join(waveform_dir, f))
        all_exps.append(exp)
...
def group_experiments(all_exps: List[MouseExp]) -> Dict[str, List[MouseExp]]:
    ...
    for exp_type in types:
        this_exps = [
            exp for exp in all_exps if exp_type_from_name(exp.basename) == exp_type
        ]
...
class XXDPrimeResult(object):
    ...
    def add_threshold(
        self, dp_criteria=2, fit_method: str = "bilinear", plot=False
    ) -> None:
        ...
        if len(self.channels) != 2:
            return np.zeros((0, 2))
        spl_threshold = np.zeros((len(self.freqs), len(self.channels)))
        ...
        smoothed = np.zeros((len(self.freqs), len(self.levels), len(self.channels)))
        ...
        for i, freq in enumerate(self.freqs):
            for j, channel in enumerate(self.channels):
                ...
                if interp and plot:
                    ...
                    plt.plot(
                        plot_levels,
                        [interp.eval(l) for l in plot_levels],
                        label=f"{channel_names[channel]} at {freq}Hz",
                        color=color_list[i],
                        ls=ls,
                    )
                    plt.plot(levels, dprimes, "x", color=color_list[i])
...
def XXcalculate_cov_dprime(
    ...
    if debug:
        data_range = (
            min(np.min(h1_response), np.min(h2_response)),
...
def XXcalculate_dprime_by_trial_count_bs(
    ...
    block_sizes = (trial_count / (2 ** np.arange(0, num_divisions, 1.0))).astype(int)
    ...
    for i, block_size in enumerate(block_sizes):
        ...
        for j in range(repetition_count):
            ...
            signal_data = filtered_abr_stack[
                freq_index,
                level_index,
                channel_index,
                :,
...
def XXcalculate_rmses(signal_data, noise_data, debug):
    ...
    if debug:
        range = (
            min(np.min(signal_rms), np.min(noise_rms)),
...
def compute_and_cache_dprimes():
    ...
    mouse_dirs = [
        os.path.basename(m) for m in find_all_mouse_directories(GeorgeMouseDataDir)
    ]
    # mouse_dirs = ['20230828']
    for dir in mouse_dirs:
        ...
        if (
            ...
            and os.path.exists(dprime_file)
            and os.path.getsize(dprime_file)
        ):
        ...
        if run_local:
            ...
            out = subprocess.run(
                ["/usr/bin/python3", cmd, f"--filter={base}", f"--basedir={full}"],
                capture_output=True,
            )
...
def filter_animals(exps: List[MouseExp], animal_list: List[str]) -> List[MouseExp]:
    ...
    def cache_waveforms(file_num: int, all_filtered_exps: List[MouseExp]) -> int:
        if len(all_filtered_exps) > 0:
            ...
            with open(good_waveform_cache, "w") as f:
                print(
                    f"Writing {len(all_filtered_exps)} waveforms to "
...
def create_stack(exps: List[MouseExp]) -> Tuple[NDArray, NDArray, 
    ...
    for i, exp in enumerate(exps):
        data[
            freqs.index(exp.freq),
            levels.index(exp.level),
            channels.index(exp.channel),
            :,
            :,
        ] = exp.single_trials
...
def show_mean_stack(stack, freq=1, channel=0, alpha=0.01):
    ...
    for i, level in enumerate(levels):
        ...
        plt.plot(np.mean(stack[freq, level, channel, ...], axis=-1), alpha=alpha)
        if i != len(levels) - 1:
...
def load_rms_data(
    base_dir: str = GeorgeMouseDataDir, cache_filename: str = "good_waveform_cache.pkl"
    ...
    if not os.path.exists(rms_cache_file):
        ...
        all_rms = [
            [[[] for _ in standard_channels] for _ in standard_levels]
            for _ in standard_freqs
        ]
        for f in glob.glob(
            os.path.join(GeorgeMouseDataDir, "good_waveform_cache*.pkl")
        ):
            with open(f, "r") as f:
                exps = jsonpickle.decode(f.read())
                (new_rms, standard_freqs, standard_levels, standard_channels) = (
                    summarize_all_rms(exps, all_rms)
                )
...
def XXload_rms_data(
    ...
    if not os.path.exists(rms_cache_file):
        ...
        with open(rms_cache_file, "r") as f:
            all_rms = jsonpickle.decode(f.read())
            print(f"Read {total_size(all_rms)} RMS results from " f"{rms_cache_file}")
    return all_rms
...
def calculate_mean_std_rms_values(
    ...
    rms_means = np.zeros(
        (len(standard_freqs), len(standard_levels), len(standard_channels))
    )
...
def calculate_dprime_by_trial_count(
    ...
    for i, block_size in enumerate(block_sizes):
        ...
        for block_start in range(0, trial_count - block_size + 1, block_size):
            ...
            signal_data = filtered_abr_stack[
                freq_index, level_index, channel_index, :, block_start:block_end
            ]
            noise_data = filtered_abr_stack[
                freq_index, noise_index, channel_index, :, block_start:block_end
            ]
...
def snr_vs_window_size(
    ...
    for i, time_start in enumerate(time_windows):
        for j, time_end in enumerate(time_windows):
            ...
            signal_data = abr_stack[
                freq_index, level_index, channel_index, time_window, :
            ]
            noise_data = abr_stack[
                freq_index, noise_index, channel_index, time_window, :
            ]
...
def stack_t_test(
    ...
    for level_index in range(filtered_abr_stack.shape[1]):
        ...
        for j, block_size in enumerate(block_sizes):
            noise_response = []
            abr_response = []
            for signal, noise in block_waveform_stack(
                filtered_abr_stack,
...
def cache_waveform_one_dir(
    ...
    Args:
      waveform_dir: Directory to find a single mouse's waveform CSV files.
      cache_dir: Where to put the Waveform CSV files, mirroring the waveform_dir.
    ...
    print(
        ...
        f"CSV waveforms in {waveform_dir} into {cache_dir}."
    )
    cache_all_mouse_dir(
        waveform_dir, cache_dir, waveform_pickle_name, 
...
def main(_):
    if FLAGS.mode == "waveforms":
      if not os.path.isdir(FLAGS.waveform_dir):
        raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), 
                                FLAGS.waveform_dir)
      if not os.path.isdir(FLAGS.cache_dir):
        raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), 
                                FLAGS.cache_dir)
      all_mouse_dirs = find_all_mouse_directories(FLAGS.waveform_dir)
      for mouse_dir in all_mouse_dirs:
