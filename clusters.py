...
"""
# spreadsheet_path_v1 = (
#     'https://docs.google.com/spreadsheets/d/1M3pM45Qb2z2pZ_liVssCpOwCVb8fHtaeu1kkiomzxWw/edit#gid=0')  # VMA Added - working

spreadsheet_path_v1 = ('https://docs.google.com/spreadsheets/d/'
                       '119_-qrfzGXwV1YBUJdBzvtAQTZnl-xwN7hD9FK5SWfU/'
                       'edit#gid=84023254')  # Old - Not working
...
def HLossClassifier(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for freq in frequencies:
        ...
        df.loc[df[right_col].notna() & df[left_col].isna(),
               left_col] = df[right_col]
    ...
    pta_cols = [['R_PTA', ['R500', 'R1000', 'R2000']],
                ...
                ['R_PTA_BC_Mod', ['RBone500', 'RBone1000', 'RBone2000']],
                ['L_PTA_BC_Mod', ['LBone500', 'LBone1000', 'LBone2000']],
                ['R_HFPTA_BC_Mod', ['RBone1000', 'RBone2000', 'RBone4000']],
                ['L_HFPTA_BC_Mod', ['LBone1000', 'LBone2000', 'LBone4000']],
...
def HLPlot(df: pd.DataFrame, title=None):
    ...
    """
    df_normal = df.loc[df['R_Type_HL_4freq'] == 'Normal']
    df_snhl = df.loc[df['R_Type_HL_4freq'] == 'SNHL']
    df_conductive = df.loc[df['R_Type_HL_4freq'] == 'Conductive']
    df_mixed = df.loc[df['R_Type_HL_4freq'] == 'Mixed']

    plt.scatter(df_normal['R_PTA_BC_4freq'], df_normal['R_PTA_4freq'], color='r',
                alpha=0.5, label='Normal')
    plt.scatter(df_snhl['R_PTA_BC_4freq'], df_snhl['R_PTA_4freq'], color='b',
                alpha=0.5, label='SNHL')
    plt.scatter(df_conductive['R_PTA_BC_4freq'], df_conductive['R_PTA_4freq'],
                alpha=0.5, color='g', label='Conductive')
    plt.scatter(df_mixed['R_PTA_BC_4freq'], df_mixed['R_PTA_4freq'],
